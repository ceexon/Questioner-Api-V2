...
class MeetupTest(BaseTest):
    ...
    def sign_login_local(self):
        ...
        self.client.post(
            "api/v2/auth/signup", data=json.dumps(payload), content_type="application/json")
        response = self.client.post(
    ...
    def test_meetups(self):
        ...
        created = json.loads(response.data.decode("utf-8", secret))
        self.assertEqual(
            created["error"], "You may be trying to duplicate a meetup, one with same time and location exists")
        ...
        """ test delete meetup """
        self.client.post(
            "api/v2/meetups", data=json.dumps(self.meetup_to_delete), headers={"x-access-token": admin_token}, content_type="application/json")
    ...
    def test_meetup_rsvp(self):
        ...
        response = self.client.post("api/v2/meetups/1/rsvp", headers={
                                    "x-access-token": admin_token}, data=json.dumps({"mangos": "fruit"}), content_type="application/json")
        error_message = json.loads(response.data.decode("utf-8"))
        ...
        response = self.client.post("api/v2/meetups/1/rsvp", headers={
                                    "x-access-token": admin_token}, data=json.dumps({"status": "fruit"}), content_type="application/json")
        error_message = json.loads(response.data.decode("utf-8"))
        self.assertEqual(
            error_message["error"], "invalid choice. Status response is limited to 'yes/maybe/no'")
        self.assertEqual(response.status_code, 400)

        """ test invalid meetup id"""
        response = self.client.post("api/v2/meetups/89/rsvp", headers={
                                    "x-access-token": admin_token}, data=json.dumps({"status": "y"}), content_type="application/json")
        error_message = json.loads(response.data.decode("utf-8"))
